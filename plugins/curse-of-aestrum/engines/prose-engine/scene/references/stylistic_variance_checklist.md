...
| S8 | **Unanchored referent at first appearance** (clarity kin of S2's ambiguous actor) | Any character, creature, or object making its **first appearance in this beat** — in narration, or as the speaker of a dialogue block | The entity introduced by **bare pronoun** (*she came around the bend*) or by **bare name / dialogue label alone**, with nothing in the introducing clause to tell the reader **which figure on the page is meant**. A bare pronoun presupposes a referent the reader already holds; a newcomer supplies none, so the pronoun attaches to whoever is nearest — and the row fires hardest when another referent of the same gender or class is live in the scene, because then it attaches to **the wrong character** for a sentence or more. Hardest of all when the anchor arrives only *downstream* — a name two sentences later, or a `**Name**` dialogue label — because by then the reader has mis-attached and has to go back and re-read. **POV-side acquaintance neither triggers nor excuses this row:** whether the anchor character knows this person on sight governs only whether their *name* may be spoken (the epistemic suite's identity-before-introduction row) and says nothing about whether the **reader** can tell who walked in. Nothing is false and nothing is repeated — the reader simply loses track of who is on the page | **Anchor the reference; do not identify the person.** Give the introducing clause a descriptor that distinguishes this figure from the others present — class plus one detail is plenty (*a woman in pale robes came around the bend*; *a man in a carter's apron*; *the tall one at the back*) — and let pronouns carry it from there. **This row never asks for a name,** and never asks narration to supply knowledge the POV lacks: a character may enter unnamed and stay unnamed for the rest of the scene or the whole work, and is fully anchored that entire time by description alone. The descriptor is in fact what satisfies **this row and row 20 at once**, which is why there is no bind between them — an unnamed stranger rendered *as* a stranger is the compliant case, not the deadlocked one. Where the POV *does* know the face, the recognition is its own beat and **follows** the description instead of replacing it (*…and then he knew the face*). A dialogue label is never permitted to be the reader's first fix on who is speaking; label an unnamed speaker with the same descriptor the narration used | [dialogue_format.md]({{PLUGIN_ROOT}}/engines/prose-engine/scene/references/dialogue_format.md) (attribution), and this file's S2 ambiguous-actor note, generalized |
...
**Read on fire (S3):** the work’s declared setting (region, period,
...
**Read on fire (S8):** the beat's **own prior sentences** — already in the draft,
no file read needed. The check is mechanical, local, and a question about
*reference* rather than identity: for each entity's first appearance, ask *can a
reader holding only this beat tell which figure on the page this refers to?*
Count the live referents a pronoun could attach to; if there is more than one, or
if the only anchor sits **downstream** of the pronoun, the row fires. An unnamed
stranger introduced by description **passes cleanly, and goes on passing for as
long as they stay unnamed** — the row is satisfied by the descriptor, never by a
name. Dialogue-block labels are scanned exactly as narration is: a label is an
anchor arriving late, not an anchor.
