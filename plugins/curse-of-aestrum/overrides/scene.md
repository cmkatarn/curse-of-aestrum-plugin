...
The thin shell handles the engine-side mechanics (`{{PLUGIN_ROOT}}/engines/rpg-5e-engine/rules`).
...
**Failure mode.** Rendering the night after first midnight as if it were an
ordinary night — NPCs walking around, watch rotations continuing, locals awake
at 2 AM — is a setting-fidelity break. Re-derive the night's world-state from
the reset sequence above before rendering it, and correct the draft before
posting.
